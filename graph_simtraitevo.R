...
ggplot(Data_storage_total,aes(x=Dispersal,y=Mean_MPD_abund,group=TEvoModel,color=TEvoModel))+
  geom_line(size=1.5)+
...
ggplot(Data_storage_total,aes(x=Mean_speciessorting,y=Mean_MPD_abund,group=TEvoModel,color=TEvoModel))+
  geom_line(size=1.5)+
  geom_ribbon(aes(ymin=Mean_MPD_abund-SD_MPD_abund,ymax=Mean_MPD_abund+SD_MPD_abund),width=0.1)+
  theme_bw(base_size = 15)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
