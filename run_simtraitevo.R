...
TraitEvo<-c("BM", "random", "conserved") #conserved hasn't been worked out yet 
...
for(i in 1:length(DispV)){
 #BIOMASS
  ...
  Data_storage$Kstat[Data_storage$Dispersal==DispV[i] & Data_storage$TEvoModel==TraitEvo[l] & Data_storage$ReplicateNum==j & Data_storage$Scale == "Regional"]<- mean(Kcalc(interspec_mat,SIH_data[["phylo",i]])) #calculating K statistic, not sure if should take the mean of all of the values 
